...
def main(request):
    if request.method=="POST":
        ...
        sk.path=path
        sk.file=file
        gsk=Query(percentage=percentage,path=path,file=file)
        sk.myfiles=call()
        num=datas()
        print(sk.myfiles)
        ...
        return render(request,'gsk.html')
...
def datas():
    # def plag_checker():# this function is neglected 
...
# defining a function to compare the strings using levenshtein's algorithm
  ...
  # defining a zero matrix of size of first string * second string
    ...
    for i in range (0,len(sk.myfiles)) :
        ...
        n=str(n)+'%'
        l.append(n)
    os.chdir(r'C:\Users\J\Desktop\Projects\gsk\plagcheck')
    return l
